...
class Pembimbing_lapangan extends CI_Controller
{
    ...
    public function detail_pbl($offset = NULL)
    {
        ...
        if (!isset($_POST['cari'])) {
            ...
            $this->cari_Data($data);
            // $this->session->set_flashdata('message', '<div class="alert alert-warning" role="alert">" SA MALAS MENCARI' . ' </div>');
        } else if (($_POST['cari'] == "")) {
            $this->load->view('data_pbl/detail_pbl', $data);
        }
    }
    ...
    private function cari_Data($dari_db)
    {
        ...
        foreach ($sql->result_array() as $as) {
            ...
            $raita = new raita;
            $strng = trim(preg_replace('/\s+/', ' ', $_POST['stringcari']));
            if ($strng == "") {
                redirect('pembimbing_lapangan/detail_pbl');
            }
            $convert_string1 = trim(preg_replace('/\s+/', ' ', $convert_string));
            $result = $raita->search(strtolower(trim($convert_string1)), strtolower($strng));
            $rekam[] = $result;
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-warning" role="alert">' . $notif . '" : "' . $strng . '"  Silahkan Cek dan Data Anda' . ' </div>');
        $data["key"] = $strng;
        // redirect('pembimbing_lapangan/detail_pbl');
